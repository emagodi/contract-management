...
import { 
  EyeIcon, DocsIcon, ArrowUpIcon, ArrowRightIcon,
  UserIcon, CalenderIcon, DollarLineIcon, InfoIcon, BoltIcon, TaskIcon
} from "@/icons";
...
export default function ApprovalQueuePage() {
  ...
  const sortedItems = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      arr = arr.filter(item => 
        ...
        (item.requisitionFrom && item.requisitionFrom.toLowerCase().includes(lower)) ||
        (item.requisitionTo && item.requisitionTo.toLowerCase().includes(lower)) ||
        (item.contractPrice && String(item.contractPrice).toLowerCase().includes(lower))
      );
    }
    ...
    return arr;
  }, [items, sortKey, sortDir, searchQuery]);
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle="Approval Queue" showTitle={false} />
          ...
          <div className="flex flex-wrap items-center gap-3 text-sm">
            ...
            <div className="flex items-center gap-2 bg-gray-50 p-1 rounded-lg border border-gray-100">
              ...
              <select 
                ...
                onChange={(e) => { const [key, dir] = e.target.value.split("_") as ["createdAt" | "id", "asc" | "desc"]; setSortKey(key); setSortDir(dir); setCurrentPage(1); }} 
                className="bg-white border border-gray-200 rounded-md px-2 py-1 text-gray-700 text-xs font-medium focus:outline-none hover:border-gray-300 transition-colors cursor-pointer"
                title="Sort order"
              >
        ...
        <div className="max-w-full overflow-x-auto">
          <div className="min-w-[980px]">
            <Table>
                ...
                <TableRow className="border-b border-emerald-100 bg-emerald-50 dark:border-white/[0.05] dark:bg-white/[0.03]">
                  <TableCell isHeader className="px-5 py-3 font-medium text-emerald-800 text-start text-xs dark:text-gray-400">
                    <div className="flex items-center gap-2">
                      <UserIcon className="w-5 h-5 shrink-0" /> From
                    </div>
                  </TableCell>
                  <TableCell isHeader className="px-5 py-3 font-medium text-emerald-800 text-start text-xs dark:text-gray-400">
                    <div className="flex items-center gap-2">
                      <UserIcon className="w-5 h-5 shrink-0" /> To
                    </div>
                  </TableCell>
                  <TableCell isHeader className="px-5 py-3 font-medium text-emerald-800 text-start text-xs dark:text-gray-400">
                    <div className="flex items-center gap-2">
                      <EyeIcon className="w-5 h-5 shrink-0" /> View Requisition
                    </div>
                  </TableCell>
                  <TableCell isHeader className="px-5 py-3 font-medium text-emerald-800 text-start text-xs dark:text-gray-400">
                    <div className="flex items-center gap-2">
                      <ArrowUpIcon className="w-5 h-5 shrink-0" /> Upload Document
                    </div>
                  </TableCell>
                  <TableCell isHeader className="px-5 py-3 font-medium text-emerald-800 text-start text-xs dark:text-gray-400">
                    <div className="flex items-center gap-2">
                      <DocsIcon className="w-5 h-5 shrink-0" /> View Attachments
                    </div>
                  </TableCell>
                  <TableCell isHeader className="px-5 py-3 font-medium text-emerald-800 text-start text-xs dark:text-gray-400">
                    <div className="flex items-center gap-2">
                      <TaskIcon className="w-5 h-5 shrink-0" /> Approval Form
                    </div>
                  </TableCell>
                </TableRow>
              </TableHeader>
              ...
              <TableBody>
                {loading ? (
                  <TableRow><TableCell className="px-5 py-8 text-center text-gray-500" colSpan={7}>
                    <div className="flex justify-center items-center">
                      <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
                    </div>
                  </TableCell></TableRow>
                ) : error ? (
                  <TableRow><TableCell className="px-5 py-8 text-center text-red-600" colSpan={7}>{error}</TableCell></TableRow>
                ) : pagedItems.length === 0 ? (
                  <TableRow>
                    ...
                    <TableRow key={r.id} className="hover:bg-blue-50/50 transition-colors group border-b border-gray-50 last:border-none">
                      ...
                      <TableCell className="px-5 py-3 text-center">
                          <button 
                            ...
                            className="p-1 rounded-md text-green-600 hover:bg-green-50 hover:text-green-700 transition-colors focus:outline-none focus:ring-1 focus:ring-green-200"
                            title="View Attachments"
                          >
                            <DocsIcon className="w-5 h-5 shrink-0" />
                          </button>
                      </TableCell>
                      <TableCell className="px-5 py-3 text-center">
                           <button 
                             onClick={() => router.push(`/requisitions/approvals/${r.id}`)} 
                             className="p-1 rounded-md text-indigo-600 hover:bg-indigo-50 hover:text-indigo-700 transition-colors focus:outline-none focus:ring-1 focus:ring-indigo-200"
