...
export default function ProcurementManagerViewForm({ requisition, onSubmit, submitting, error, onCancel }: {
  ...
  onSubmit: (decision: "YES" | "NO", signaturePath?: string) => void;
  submitting?: boolean;
  error?: string | null;
  onCancel: () => void;
}) {
  ...
  const pmDate = decision ? formatDate(today) : "";
  ...
  useEffect(() => {
    ...
    if (typeof requisition.headOfDept === "string" && requisition.headOfDept.startsWith("/api/v1/signature")) {
  ...
  // Fetch FD Signature
  useEffect(() => {
    if (typeof requisition.financeDirector === "string" && requisition.financeDirector.startsWith("/api/v1/signature")) {
      setFdSignatureUrl(`http://localhost:8080${requisition.financeDirector}`);
    }
  }, [requisition]);

  // Fetch Procurement Manager Signature (Current User)
  useEffect(() => {
    ...
    const savedSignature = requisition.procurementManager;
    if (savedSignature && savedSignature.startsWith("/api/v1/signature")) {
      ...
      setDecision(requisition.procurementComplied as "YES" | "NO" || "");
    } else {
      fetchSignature();
    }
  }, [requisition]);
    ...
    <div className="max-w-5xl mx-auto mt-8 bg-white border border-gray-200 shadow-2xl rounded-xl p-10 text-gray-800 font-sans">
      ...
      <div className="border border-gray-200 rounded-lg overflow-hidden shadow-sm transition-shadow hover:shadow-md bg-white mb-6">
        ...
        <div className="p-6 grid grid-cols-[250px_1fr] gap-y-4 text-sm">
          ...
          <div className="flex flex-wrap items-center gap-4">
            <Input readOnly value={val("deliveryDays")} className={readOnlyClass + " w-16"} placeholder="Days" />
            <Input readOnly value={val("deliveryWeeks")} className={readOnlyClass + " w-16"} placeholder="Wks" />
            <Input readOnly value={val("deliveryMonths")} className={readOnlyClass + " w-16"} placeholder="Mths" />
            <label className="flex items-center gap-2 text-gray-600 ml-4"><input type="radio" disabled checked={val("deliveryNA") === "YES"} /> N/A</label>
          </div>
          ...
          <div className="text-gray-600 font-medium flex items-center pt-2">Acceptance Testing</div>
      ...
      <div className="border border-gray-200 rounded-lg overflow-hidden shadow-sm transition-shadow hover:shadow-md bg-white mb-6">
        ...
        <div className="p-6 grid grid-cols-[250px_1fr] gap-y-4 text-sm">
          ...
          <div className="flex flex-wrap items-center gap-4">
            <Input readOnly value={val("warrantyDays")} className={readOnlyClass + " w-16"} placeholder="Days" />
            <Input readOnly value={val("warrantyWeeks")} className={readOnlyClass + " w-16"} placeholder="Wks" />
            <Input readOnly value={val("warrantyMonths")} className={readOnlyClass + " w-16"} placeholder="Mths" />
            <label className="flex items-center gap-2 text-gray-600 ml-4"><input type="radio" disabled checked={val("warrantyNA") === "YES"} /> N/A</label>
          </div>
      ...
      <div className="border border-gray-200 rounded-lg overflow-hidden shadow-sm transition-shadow hover:shadow-md bg-white mb-6">
        ...
        <div className="p-6">
          ...
          <p className="italic text-xs text-gray-400 mt-2 text-right">Specific conditions to be incorporated in the contract</p>
        </div>
      </div>
        ...
        <div className="p-6 space-y-8 bg-gray-50/50">
          ...
          <div className="flex justify-between items-end">
            <div className="text-gray-700 text-sm mb-1 font-medium flex items-center gap-2">
               ...
               <div className="flex gap-4 ml-4">
                 <label className={`cursor-pointer px-2 py-1 rounded border text-xs transition-all ${decision === 'YES' ? 'bg-blue-50 border-blue-500 font-bold text-blue-700' : 'bg-white border-gray-200 hover:border-blue-300'}`}>
                   <input type="radio" className="hidden" name="pmDecision" value="YES" checked={decision === 'YES'} onChange={() => setDecision('YES')} />
                   Yes [ {decision === 'YES' ? '✓' : '\u00A0'} ]
                 </label>
                 <label className={`cursor-pointer px-2 py-1 rounded border text-xs transition-all ${decision === 'NO' ? 'bg-red-50 border-red-500 font-bold text-red-700' : 'bg-white border-gray-200 hover:border-red-300'}`}>
                   <input type="radio" className="hidden" name="pmDecision" value="NO" checked={decision === 'NO'} onChange={() => setDecision('NO')} />
                   No [ {decision === 'NO' ? 'X' : '\u00A0'} ]
                 </label>
               </div>
            </div>
            ...
            <div className="flex gap-10 text-center">
              <div className="flex flex-col items-center group relative">
                <div className={`w-64 border-b-2 border-dotted mb-1 h-12 flex items-end justify-center relative transition-all duration-300 rounded-lg ${!signatureUrl ? 'border-gray-300 hover:border-blue-500 hover:bg-blue-50 cursor-pointer' : 'border-gray-400'}`}>
                   {signatureUrl ? (
                     <div className="relative w-full h-full flex items-end justify-center">
                       <Image src={signatureUrl} alt="My Signature" width={120} height={60} className="object-contain max-h-12 absolute bottom-0" />
                       <button 
                        onClick={(e) => { 
                          e.preventDefault();
                ...
                <div className="font-bold text-gray-500 text-xs uppercase tracking-wider group-hover:text-blue-600 transition-colors mt-2">Procurement Manager</div>
              </div>
              <div className="flex flex-col items-center group">
                <div className="w-32 border-b-2 border-dotted border-gray-400 mb-1 h-12 flex items-end justify-center font-mono text-sm font-bold text-gray-800">
                  {decision ? formatDate(today) : ""}
                </div>
                <div className="font-bold text-gray-500 text-xs uppercase tracking-wider mt-2">Date</div>
              </div>
            </div>
      ...
      <div className="mt-8 flex gap-6 pt-6 border-t border-gray-100">
        <Button 
          ...
          onClick={() => decision && onSubmit(decision as "YES" | "NO", signaturePath || undefined)} 
          disabled={submitting || !decision || !signatureUrl}
        >
          {submitting ? (
            <>
